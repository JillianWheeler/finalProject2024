...
int calculationCount = 0; // Variable to track the number of calculations
...
void *multiply(void *arg) {
    ...
    int partialCalculation = 0;

    for (int i = 0; i < MATRIX_SIZE; i++) {
        partialCalculation += matrixA[data->row][i] * matrixB[i][data->col];
    }
     // Lock the mutex
    pthread_mutex_lock(&countMutex);
    calculationCount++; // Increment calculationCount
    // Unlock the mutex 
    pthread_mutex_unlock(&countMutex);

    resultMatrix[data->row][data->col] = partialCalculation;
