...
void getLocalTime(time_t *local_time) {
    time(local_time);
}

void getNTPTime(time_t *ntp_time) {
    int sockfd, n;
...
int main() {
    ...
    // Get NTP time
    getNTPTime(&ntp_time);
    ntp_tm = localtime(&ntp_time);

    // Print local time
    printf("Local time: %s", asctime(local_tm));

    // Print NTP time
    printf("NTP time  : %s", asctime(ntp_tm));

    // Print time difference
    printf("Time difference: %ld seconds\n", (long)(ntp_time - local_time));
